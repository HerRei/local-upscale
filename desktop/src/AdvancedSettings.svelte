    ...
    <div class="advanced-controls">
      ...
      {:else}
        ...
        {#if !usingTemporalVideo}
          <label class="check-row"><input type="checkbox" disabled={settings.video_hdr_mode === 'preserve'} checked={settings.deflicker} on:change={(event) => updateSettings({ deflicker: event.currentTarget.checked })} /> De-flicker · Labs</label>
        ...
        {/if}
        <p class="model-description">Original timing and orientation preserved. HLG/PQ output follows the HDR setting above; HDR uses FP32 and disables de-flicker.</p>
      {/if}
