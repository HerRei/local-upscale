...
>
  <div bind:this={viewport} class="video-viewport">
    <video bind:this={original} src={originalSrc} muted playsinline preload="auto" on:loadedmetadata={updateDuration} on:loadeddata={() => (originalLoaded = true)} on:error={(event) => mediaError('Original', event)}></video>
    <div class="enhanced-clip" style={`clip-path: inset(0 ${100 - compare}% 0 0)`}>
      ...
      <video bind:this={enhanced} src={enhancedSrc} playsinline preload="auto" on:loadedmetadata={updateDuration} on:loadeddata={() => (enhancedLoaded = true)} on:timeupdate={updateTime} on:ended={() => { playing = false; original.pause(); }} on:pause={() => (playing = false)} on:play={() => (playing = true)} on:error={(event) => mediaError('Enhanced', event)}></video>
    </div>
  ...
  {:else if !originalLoaded || !enhancedLoaded}
    <div class="video-loading" role="status"><i aria-hidden="true"></i>Loading video comparison…</div>
  {/if}
...
<style>
  ...
  .video-loading { position: absolute; z-index: 7; left: 50%; top: 50%; transform: translate(-50%, -50%); display: flex; align-items: center; gap: 10px; padding: 12px 16px; border-radius: 8px; background: #171e30ed; color: #c3d1fa; pointer-events: none; }
  .video-loading i { width: 16px; height: 16px; border: 2px solid #a6c0ff40; border-top-color: #b9ccff; border-radius: 50%; animation: loading-spin 1s linear infinite; }
  @keyframes loading-spin { to { transform: rotate(360deg); } }
  @media (prefers-reduced-motion: reduce) { .video-loading i { animation: none; } }
</style>
