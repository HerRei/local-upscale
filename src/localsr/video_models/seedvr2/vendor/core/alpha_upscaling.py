...
# Modified for LocalSR: cv2 is imported lazily inside the functions that
# need it, so RGB-only installs do not require OpenCV.
import numpy as np
...
def detect_edges_batch(
    ...
    for t in range(T):
        ...
        if method == 'sobel':
            sobelx = _cv2().Sobel(gray, _cv2().CV_64F, 1, 0, ksize=3)
            sobely = _cv2().Sobel(gray, _cv2().CV_64F, 0, 1, ksize=3)
