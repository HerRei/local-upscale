...
def estimate_resources(
    *,
    ...
    base_system_memory = int(
        input_float_bytes
        + output_bytes
        + min(output_bytes, 512 * 1024 * 1024)
        + model_file_size * 1.2
        + 256 * 1024 * 1024
    )
    ...
    system_memory = (
        base_system_memory + device_memory if device_type in {"cpu", "mps"} else base_system_memory
    )
    working_disk = int(output_bytes * 2.1 + 64 * 1024 * 1024)
    ...
    if measured_seconds_per_megapixel and measured_seconds_per_megapixel > 0:
        ...
        baseline = {"cuda": 18.0, "mps": 38.0, "cpu": 180.0}.get(device_type, 75.0)
        fixed_overhead = {"cuda": 4.0, "mps": 8.0, "cpu": 12.0}.get(device_type, 8.0)
    ...
    if (
        ...
        warnings.append(
            f"Estimated VRAM use ({format_bytes(device_memory)}) exceeds safe free VRAM "
            f"({format_bytes(available_device_memory)}). Choose a smaller tile or CPU."
        )
        blocking = True
    elif (
        device_type == "cuda"
        ...
        warnings.append("VRAM will be tight; Safe Memory Mode is recommended.")
    ...
    if device_type == "mps" and available_device_memory:
        ...
        if system_memory > unified_available * 0.9:
            warnings.append(
                f"Estimated unified-memory use ({format_bytes(system_memory)}) is too close to "
                f"available memory ({format_bytes(unified_available)}). Choose a smaller tile."
            )
            blocking = True
        elif system_memory > unified_available * 0.7:
            warnings.append("Unified memory will be tight; Safe Memory Mode is recommended.")
