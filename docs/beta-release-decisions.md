...
| B2 | Individual or registered-company publisher | Record whether LocalSR is published by the individual Hermes Reisner (display name HerRei) or a registered company; apply the answer to the prepared publisher details | Next walkthrough question; awaiting answer |
...
| B18 | Native Windows Intel test device | User authorized SSH, Tailscale and full GUI access for native Intel/iGPU and Store/MSIX testing; the device must not become a build runner | SSH, file transfer and desktop input verified over LAN and Tailscale; secure-desktop UAC handling verified over LAN. Automatic sleep disabled as requested. Native Intel inference and MSIX acceptance remain queued on the to-do list. [Setup and hardware evidence](windows-intel-test-host.md). Continue the walkthrough with B2 |
...
The user then explicitly retained the full set of backends and accepted Labs
labels for the less-tested paths, identifying macOS MPS, Linux ROCm and Windows
CPU/Intel iGPU as the main tested paths. Broad physical GPU coverage is not a
prerequisite for including a backend as Labs. The saved records establish
bounded MPS checks, installed Linux CPU/ROCm checks and Windows CPU-worker
checks. An Intel iGPU run is user-reported but not identified in the saved
Windows record, so its hardware/build evidence remains to capture. The
[coverage matrix](beta-platform-matrix.md) preserves all eight targets and
separates actual evidence from the planned testing focus. No result was marked
passed and no `.12` configuration was changed by this decision.
